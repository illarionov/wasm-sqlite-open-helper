...
__BundledSQLiteDriver__ from the [androidx.sqlite] library is a special build of the SQLite for use in Kotlin 
Multiplatform projects from the Android Jetpack team.  
In this article, we will look at its features compared to the built-in Android SQLite.
...
Then on April 9, 2025, [Room 2.7.0] — the first stable version with Multiplatform support — was released.
...
This separation (following an API – Implementation style) allows replacing the version of SQLite used together with Room.
The main implementation — the `FrameworkSQLiteOpenHelper` class — is located in the `androidx.sqlite:sqlite-framework` module.

Examples of third‑party libraries that implement these APIs and offer their own variants of SQLite include
[SQLCipher] and [requery/sqlite‑android].

Moreover, this interfaces are used not only in Room but also, for example, in the [SQLDelight] library.
...
In general, the _SupportSQLite\*_ interfaces mimicked the Android platform classes _SQLiteOpenHelper_, _SQLiteDatabase_,
_SQLiteStatement_.
...
In version 2.5.0, the `androidx.sqlite` library was ported to KMP.
The original `SupportSQLite*` interfaces were retained in the Android variant of the library for compatibility, 
and a new interfaces was added in common source set:
...
* `SQLiteStatement` — an abstraction over SQLite Prepared Statement used to execute queries.

This new variant is much closer to the SQLite C API. In short, it can be described as follows:
...
* __[AndroidSQLiteDriver]__ — an implementation based on Android’s SDK SQLite APIs (Android‑only).
...
* __[BundledSQLiteDriver]__ — a custom build of SQLite which is packaged with the application. This is the recommended variant, suitable for Android, iOS, Mac, Linux, and JVM.

Although `androidx.sqlite` is positioned as a base for building more complex frameworks, it can also be used as a standalone library.
Now that this is a Kotlin Multiplatform project, we are not limited to Android only.
For example, we can use this library on the JVM host instead of SQLite JDBC or even in Gradle plugins to prepare 
...
With __AndroidSQLiteDriver__, the SQLite version used depends on the Android version of the device — and usually it is not the latest.
...
3. __Performance__  
The performance is generally higher, not only because of the newer SQLite version. In Android’s system SQLite,
classes originally designed for interprocess communication are used. Data transfer between native memory and the 
JVM is implemented based on a fixed‑size buffer (usually 2 MB), represented on the Java side by the class
[CursorWindow] and on the native side by the [C++ class CursorWindow][cppCursorWindow]. In the past, this buffer caused many problems for developers using SQLite on Android. Now, this buffer is no longer necessary.
...
The build configuration of __BundledSQLiteDriver__ differs from that used in Android’s SQLite.
...
Those who have migrated from the Android‑version likely noticed that SQLite in BundledSQLiteDriver is built 
...
If the application has a small number of languages, a workaround can be to convert all texts to the same 
...
In __BundledSQLiteDriver__, all Android‑specific extensions are missing:

* For `COLLATE`, only the standard options — `BINARY`, `RTRIM`, and `NOCASE` — are available,
while Android‑specific ones — `LOCALIZED`, `UNICODE`, and the (undocumented) `PHONEBOOK` — are not available.
* All undocumented Android functions are unavailable:
`PHONE_NUMBERS_EQUAL()`, `_PHONE_NUMBER_STRIPPED_REVERSED()`, `_DELETE_FILE()` (in case anyone was even aware of their existence).
...
byte-code of the SQL query and the tables used:
...
* The following PRAGMA commands have been removed: *count_changes*, *data_store_directory*, *default_cache_size*,
*empty_result_callbacks*, *full_column_names*, *short_column_names*, and *temp_store_directory*.
* Shared cache has been removed.
* The `SQLITE_ALLOW_ROWID_IN_VIEW` option is disabled.
...
Check https://www.sqlite.org/json1.html for details.

#### 3. R\*-Tree Index

This is an algorithm designed for spatial data indexing. The index can be used to optimize 
...
The query of the form `ALTER TABLE DROP COLUMN` is supported, allowing the deletion of columns that are not 
...
The `ANALYZE` command is executed automatically given that the `PRAGMA optimize` command is run periodically.
It is worth reviewing the official documentation on how often to run this command .
...
More details can be found in SQLite’s Date/Time Functions documentation https://www.sqlite.org/lang_datefunc.html#uepch

#### 11. Additional Functions and Operators

* Functions `concat()` / `concat_ws(SEP, …)`.
* `string_agg()` – a synonym for `group_concat()`.
* Aggregate functions now support an `ORDER_BY` clause; this is particularly useful for `string_agg()` and `json_group_array()`.
* Operators `IS DISTINCT FROM` and `IS NOT DISTINCT FROM` — synonyms for IS NOT and IS, respectively.
* `printf()` – a synonym for `format()`.
* `unhex() `– returns a BLOB from a hexadecimal string.
* `octet_length()` — returns the number of bytes required to store the textual representation of a number in the current encoding.
...
See https://www.sqlite.org/stricttables.html for details.
...
on Android device.
It locks CPU frequencies, set process priorities, enforce AOT compilation using `CompilationMode.FULL`,
...
* `select_with_paging`  
   ...
   clause and uses paging with `OFFSET n LIMIT m`. The queries return sets of records spanning several columns.
* `huge_select`  
  A single complex query is executed on the database, taking a significant amount of time to complete as it 
  includes complex groupings and filters.
...
All tests run 20–28% faster with __BundledSQLiteDriver__ compared to __AndroidSQLiteDriver__.
